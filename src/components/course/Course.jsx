...
const Course = () => {
  ...
  return (
    <div className="min-h-screen text-white bg-[#1e293b] dark:bg-[#121212]">
      {/* Mobile Button */}
      <div className="md:hidden flex justify-end px-4 mt-4">
        <Button
          onClick={() => setIsMobileDrawerOpen(true)}
          className="bg-gradient-to-r from-purple-500 to-cyan-400 hover:from-purple-600 hover:to-cyan-500 rounded-full text-lg"
        >
          <FaFolderOpen size={20} />
        </Button>
      </div>

      <div className="w-11/12 mx-auto grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 pt-10">
        {/* Video Section */}
        <main className="md:col-span-2 lg:col-span-3">
          <div className="rounded-lg bg-[#1E293B] dark:bg-[#2d2d2d] shadow-md p-6 flex flex-col items-center overflow-auto">
          {videoUrl ? (
        ...
        {/* Sidebar */}
        <aside className="hidden md:block md:col-span-1 rounded-lg bg-[#1E293B] dark:bg-[#2d2d2d] shadow-md p-4 space-y-4 h-[100vh] overflow-y-auto">
          ...
          {course.map((section, index) => (
            <div key={index} className="border border-white/10 rounded-md overflow-hidden">
              <button
                ...
                className="w-full flex justify-between items-center px-4 py-2 text-left bg-white/5 hover:bg-white/10 transition-all"
              >
      ...
      {isMobileDrawerOpen && (
        <Dialog open={isMobileDrawerOpen} onOpenChange={setIsMobileDrawerOpen}>
          <Dialog.Content className="fixed bottom-0 left-0 right-0 h-[60vh] bg-[#1E293B] dark:bg-[#2d2d2d] z-50 rounded-t-2xl shadow-2xl p-4 overflow-y-auto transition-all">
